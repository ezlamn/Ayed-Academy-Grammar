...
function openPathMap() {
  ...
  const nodes = units.map((u, i) => {
    ...
    const lockedByProgress = i > maxUnlocked && !isDone;
    const lockedByGuest = isGuest && i >= 2;
    const locked = lockedByProgress || lockedByGuest;
    const state = isDone && !lockedByGuest ? 'done' : locked ? 'locked' : (isCurrent ? 'current' : 'open');
    return `
      <div class="nb-path-node ${state}" data-idx="${i}" data-locked="${locked ? 1 : 0}" data-guest-lock="${lockedByGuest ? 1 : 0}" style="--i:${i}">
        <div class="nb-path-dot">${isDone && !lockedByGuest ? '✓' : locked ? '🔒' : (u.emoji || u.id)}</div>
  ...
  document.querySelectorAll('.nb-path-node').forEach(node => {
    node.addEventListener('click', () => {
      if (node.dataset.locked === '1') {
        ...
        const isGuestLock = node.dataset.guestLock === '1';
        if (isGuestLock) {
          if (typeof showToast === 'function') showToast('🔒', 'هذا المحتوى متاح للمشتركين فقط. يرجى تسجيل الدخول.');
        } else {
...
/* ================================================================
   MISTAKE BOX REVIEW
   ================================================================ */
window.openMistakesBox = async function() {
  if (!window.dataService) return;
  const mistakes = await window.dataService.getMistakes() || [];
  if (mistakes.length === 0) {
    if (typeof showToast === 'function') showToast('✅', 'صندوق الأخطاء فارغ، أحسنت!');
    return;
  }

  // Build the list
  let html = `<div style="max-height: 60vh; overflow-y: auto; text-align: right; padding-right: 10px;">`;
  
  mistakes.forEach((m, idx) => {
    // Attempt to find the question text
    let qText = 'سؤال غير معروف';
    const trackData = GS.ALL_DATA[m.trackId] || [];
    
    // Quick search
    const searchObj = (obj) => {
      if (typeof obj === 'object' && obj !== null) {
        if (obj.id === m.id || obj.passageId === m.id) {
          if (obj.q) qText = obj.q;
        }
    ...
    const trackNames = { reading: 'فهم المقروء', grammar: 'التراكيب', listening: 'الاستماع', composition: 'التحليل الكتابي' };
    const tName = trackNames[m.trackId] || m.trackId;

    html += `
      <div style="background: var(--bg-sec); padding: 1rem; border-radius: var(--radius); margin-bottom: 1rem; border: 1px solid var(--border);">
        <div style="font-size: 0.85rem; color: var(--text-sec); margin-bottom: 0.5rem;">مسار ${tName}</div>
        <div style="font-size: 1.1rem; margin-bottom: 1rem; font-weight: 500;" dir="ltr">${qText}</div>
        <button class="btn btn-primary btn-sm" onclick="nbCloseModal(); window.goToUnitFromMistake('${m.trackId}', '${m.unitId}')">اذهب للمراجعة</button>
      </div>
    `;
  });

  html += `</div>`;
  nbOpenModal('صندوق الأخطاء 🚨', html);
};

window.goToUnitFromMistake = async function(trackId, unitId) {
  GS.currentTrack = trackId;
  GS.UNITS = await window.dataService.getTrackIndex(trackId);
  const uIdx = GS.UNITS.findIndex(u => String(u.id) === String(unitId));
  GS.currentUnit = uIdx >= 0 ? uIdx : 0;
  
  document.getElementById('main-dashboard').classList.add('hidden');
  document.getElementById('app').classList.remove('hidden');
  
  if (typeof initApp === 'function') initApp();
  else if (typeof loadUnit === 'function') loadUnit(GS.currentUnit);
};
  ...
  return `
    <div class="mini-q nb-order-q" id="mq-wrap-${qid}" data-answer="${encodeURIComponent(correct.join(' '))}">
      ...
      <div class="mini-q-actions">
        <button class="btn btn-primary btn-check-ans nb-check-order hidden" id="check-${qid}" data-qid="${qid}" data-realid="${pq.id || ''}">تحقق من الإجابة</button>
      </div>
  ...
  return `
    <div class="mini-q nb-fill-q" id="mq-wrap-${qid}" data-answer="${encodeURIComponent(JSON.stringify(Array.isArray(pq.answer) ? pq.answer : [pq.answer]))}">
      ...
      <div class="nb-fill-row">
        ...
        <button class="btn btn-primary nb-check-fill" id="check-${qid}" data-qid="${qid}" data-realid="${pq.id || ''}">تحقق</button>
      </div>
      <div class="mini-expl" id="mq-expl-${qid}"><div class="expl-text">${pq.expl || ''}</div></div>
    </div>`;
}
...
function bindNewQuestionTypes(container) {
  ...
  container.querySelectorAll('.nb-order-q').forEach(wrap => {
    ...
    check.addEventListener('click', () => {
      ...
      nbFinishInteractive(ok, check.dataset.realid);
    });
  });
...
function nbFinishInteractive(ok, realid) {
  if (ok) {
    if (window.NBSound) NBSound.correct();
    if (typeof registerAnswer === 'function') registerAnswer(true, null);
    if (window.dataService && realid) window.dataService.removeMistake(realid);
    ...
    if (window.dataService && realid) window.dataService.logMistake(realid, GS.currentTrack, GS.currentUnit);
