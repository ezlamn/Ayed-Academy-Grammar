...
function renderTreeDiagram(content) {
  ...
  } else if (content.formulas) {
    // New fallback for Tenses unit formula tables
    html += buildFormulasTable(content.formulas);
  }
...
// ── BUILD FROM FORMULAS ARRAY (Tenses, Present Simple, etc.) ───
function buildFormulasTable(formulas) {
  if (!formulas || !formulas.length) return '';
  const rows = formulas.map((f, i) => {
    const isSpecial = ['Negative','Question','Interrogative'].some(k => (f.subj || '').includes(k));
    const exHtml = f.ex ? `<div class="ft-example" dir="ltr">${f.ex}</div>` : '';
    return `
      <div class="ft-row${isSpecial ? ' ft-row-special' : ''}">
        <div class="ft-subj">${f.subj || ''}</div>
        <div class="ft-arrow">→</div>
        <div class="ft-form">
          <span class="ft-form-text" dir="ltr">${f.form || ''}</span>
          ${exHtml}
        </div>
      </div>`;
  }).join('');
  return `<div class="ft-table"><div class="ft-rows">${rows}</div></div>`;
}
...
function buildFromConfig(cfg, keywords) {
  ...
  (cfg.groups || []).forEach(group => {
    ...
    // Auxiliary Node
    const auxVal = group.auxiliary || '';
    const auxLower = auxVal.toLowerCase().trim();
    let auxClass = 'aux-generic';
    if (['was','were','have','has','had','will','am','is','are','do','does','did'].includes(auxLower)) {
      auxClass = `aux-${auxLower}`;
    }
    flowHtml += `<div class="cg-node cg-node-aux ${auxClass}">${auxVal}</div>`;
