...
function formatPassageText(text) {
  ...
  return lines.map(line => {
    line = line.trim();
    if (!line) return '<div style="height: 0.5rem;"></div>'; // Preserve spacing for empty lines
    
    // Bold and styled title
    if (line.toUpperCase().includes('PASSAGE')) {
      return `<div style="font-weight:900; color:var(--text-dark); font-size:1.3rem; margin-bottom:1rem; text-align:center; text-transform:uppercase; border-bottom: 2px solid var(--primary); padding-bottom: 0.5rem; display: inline-block; width: 100%;">${line}</div>`;
    }

    // Highlight paragraph numbers: (1), [1], 1., 1- at the start of the line
    line = line.replace(/^(\(\d+\)|\[\d+\]|\d+\.|\d+-)\s*/, '<span style="color:var(--danger, #e63946); font-weight:900; font-size: 1.1rem; margin-right:0.5rem;">$1</span> ');

    // Use LTR for english passages
    const isEnglish = /[a-zA-Z]/.test(line);
    const dirStr = isEnglish ? 'direction: ltr; text-align: left;' : 'direction: rtl; text-align: right;';

    return `<div style="margin-bottom:0.5rem; line-height:1.8; font-size: 1.05rem; ${dirStr}">${line}</div>`;
  }).join('');
}
  ...
  const practiceHtml = (strat.practice && strat.practice.length > 0) ? `
    ...
    return `
          <div class="mini-q" id="mq-wrap-${qid}">
            ...
            ` : ''}
            ${pq.passageText ? `
              <div class="passage-box" style="background:var(--surface2); padding:1rem; border-radius:var(--r-md); margin-bottom:1rem; font-family:var(--ff-en); white-space:pre-wrap; direction:ltr; text-align:left; color:var(--text-primary); border:1px solid var(--border);">
                ${pq.passageText}
              </div>
            ` : ''}
            ${pq.imgUrl ? `
              <div class="media-box" style="margin-bottom:1rem; text-align:center;">
                <img src="${pq.imgUrl}" alt="Passage Image" style="max-width:100%; border-radius:var(--r-md); box-shadow:var(--sh-sm);">
              </div>
            ` : ''}
            ...
            <div class="mini-q-actions">
              <button class="btn btn-primary btn-check-ans hidden" id="check-${qid}" data-qid="${qid}" data-realid="${pq.id || ''}" data-correct="${pq.c}">تحقق من الإجابة</button>
            </div>
      ...
      practiceHtml = `
        <div class="mini-quiz-wrap" style="margin-top: 1rem;">
          ...
          ${(() => {
            ...
            s.practice.forEach((pq, pi) => {
              const pHash = (pq.passageText || '') + (pq.imgUrl || '') + (pq.layout || 'auto');
              let group = groupedPractice.find(g => g.hash === pHash);
              if (!group) {
                group = { hash: pHash, passageText: pq.passageText, imgUrl: pq.imgUrl, layout: pq.layout || 'auto', questions: [] };
                groupedPractice.push(group);
              }
              pq.globalIndex = pi;
              group.questions.push(pq);
            });

            return groupedPractice.map(group => {
              const renderQs = group.questions.map(pq => {
                const pi = pq.globalIndex;
                const qid = `${s.id}-p${pi}`;
                return `
                  <div class="mini-q-box" id="mq-wrap-${qid}" style="margin-bottom: 2.5rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--border);">
                    <div class="rd-split-qnum" dir="ltr" style="text-align:left; font-weight:bold; margin-bottom:1rem; color:var(--text-primary); font-size:1.1rem;">${pi + 1}. ${pq.q}</div>
              ...
              let finalLayout = 'split';
              if (group.layout === 'stacked') finalLayout = 'stacked';
              else if (group.layout === 'split') finalLayout = 'split';
              else {
                if (group.imgUrl) finalLayout = 'stacked';
              }

              if (finalLayout === 'stacked') {
                return `
                  <div class="rd-stacked-wrap" style="margin-bottom:2rem;">
                    <div class="rd-passage" dir="ltr" style="text-align:left; background:var(--bg-sec); padding:1.5rem; border-radius:12px; margin-bottom:1.5rem; border:1px solid var(--border);">
                      ${group.passageText ? `<div class="rd-passage-text" style="margin-bottom:1rem; font-size:1.1rem; line-height:1.6;">${group.passageText.replace(/\\n/g, '<br>')}</div>` : ''}
                      ${group.imgUrl ? `<img class="rd-passage-img" src="${group.imgUrl}?v=2" alt="Passage Image" style="max-width:100%; border-radius:8px;">` : ''}
                    </div>
                ...
                return `
                  <div class="rd-split-wrap">
                    <div class="rd-split-left" dir="ltr" style="text-align:left;">
                      <div class="rd-passage">
                        ${group.passageText ? `<div class="rd-passage-text">${group.passageText.replace(/\\n/g, '<br>')}</div>` : ''}
                        ${group.imgUrl ? `<img class="rd-passage-img" src="${group.imgUrl}?v=2" alt="Passage Image">` : ''}
                      </div>
                    </div>
...
function bindInteractiveElements() {
  ...
  pc.querySelectorAll('.btn-check-ans').forEach(btn => {
    btn.addEventListener('click', function (event) {
      ...
      if (selected === correct) {
        if(typeof FX !== 'undefined' && FX.correct) { FX.correct.currentTime = 0; FX.correct.play().catch(e => console.log(e)); }
        if(typeof registerAnswer === 'function') registerAnswer(true, event);
        if(window.dataService && this.dataset.realid) window.dataService.removeMistake(this.dataset.realid);
        ...
        if(typeof FX !== 'undefined' && FX.wrong) { FX.wrong.currentTime = 0; FX.wrong.play().catch(e => console.log(e)); }
        if(typeof registerAnswer === 'function') registerAnswer(false, event);
        if(window.dataService && this.dataset.realid) window.dataService.logMistake(this.dataset.realid, GS.currentTrack, GS.currentUnit);
